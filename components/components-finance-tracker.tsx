...
export default function FinanceTracker() {
  ...
  const filteredTransactions = transactions.filter((transaction) => {
    ...
    const matchesDate = !transactionDateFilter || transaction.date === transactionDateFilter
  ...
  const getSpendingMoodData = () => {
    if (spendingPeriod === 'weekly') {
      ...
      transactions.forEach((transaction) => {
        ...
        if (txDate >= monday && txDate < end) {
          const idx = (txDate.getDay() + 6) % 7
          data[idx] += Math.abs(transaction.amount)
        }
      })
    ...
    transactions.forEach((transaction) => {
      ...
      if (txDate.getFullYear() === currentYear) {
        data[txDate.getMonth()] += Math.abs(transaction.amount)
      }
    })
  ...
  return (
    <div className="min-h-screen bg-white text-pink-700">
            ...
            <Card className="w-full max-w-md">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="chequingBalance">Initial Chequing Balance (Rs)</Label>
                    <Input
                      id="chequingBalance"
                      type="text"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="savingsBalance">Initial Savings Balance (Rs)</Label>
                    <Input
                      id="savingsBalance"
                      type="text"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="monthlyExpenses">Monthly Expenses (Rs)</Label>
                    <Input
                      id="monthlyExpenses"
                      type="text"
      ...
      <header className="sticky top-0 z-40 bg-white shadow-sm border-b border-pink-200">
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        >
          <h1 className="text-4xl font-bold mb-8">Hi {userData.name || 'bestie'} 💕 here&apos;s your money vibe today</h1>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card className="shadow-sm rounded-2xl bg-white border border-pink-200">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          <TabsList className="grid w-full grid-cols-7 bg-pink-50 border border-pink-200">
            <TabsTrigger value="dashboard" className="data-[state=active]:bg-pink-500 data-[state=active]:text-white text-pink-700">Dashboard</TabsTrigger>
            <TabsTrigger value="transactions" className="data-[state=active]:bg-pink-500 data-[state=active]:text-white text-pink-700">Transactions</TabsTrigger>
            <TabsTrigger value="goals" className="data-[state=active]:bg-pink-500 data-[state=active]:text-white text-pink-700">Goals</TabsTrigger>
            <TabsTrigger value="budget" className="data-[state=active]:bg-pink-500 data-[state=active]:text-white text-pink-700">Budget</TabsTrigger>
            <TabsTrigger value="portfolio" className="data-[state=active]:bg-pink-500 data-[state=active]:text-white text-pink-700">Portfolio</TabsTrigger>
            <TabsTrigger value="crypto" className="data-[state=active]:bg-pink-500 data-[state=active]:text-white text-pink-700">Crypto</TabsTrigger>
            <TabsTrigger value="insights" className="data-[state=active]:bg-pink-500 data-[state=active]:text-white text-pink-700">Insights</TabsTrigger>
          </TabsList>
              ...
              <Card>
                ...
                <CardContent className="relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <IndianRupee className="h-12 w-12 text-pink-300/70" />
                  </div>
                </CardContent>
              </Card>
              <Card>
                ...
                <CardContent>
                  <ScrollArea className="h-[300px]">
                    {generateBudgetRecommendations().map((recommendation, index) => (
                      <div key={index} className="flex items-start space-x-4 mb-4 bg-pink-50 border border-pink-200 p-4 rounded-lg">
                        <div className="flex-shrink-0 mt-1">
                          {recommendation.icon}
                        </div>
                        <div>
                          <h3 className="font-semibold text-lg mb-1 text-pink-700">{recommendation.title}</h3>
                          <p className="text-sm text-pink-600">{recommendation.description}</p>
                        </div>
                      </div>
                    ))}
                  </ScrollArea>
          ...
          <TabsContent value="transactions">
            <Card>
              <CardHeader>
                <CardTitle>Transactions Journal</CardTitle>
                <CardDescription>Track your money story, one cute entry at a time</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid gap-4 mb-4">
                  <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                    <Input
                      placeholder="search your chaos 💅"
                    ...
                    />
                    <Button onClick={() => setShowAddTransactionModal(true)} className="bg-pink-500 hover:bg-pink-600 text-white">
                      Add Transaction ✨
                    </Button>
                  </div>
                </div>
                <ScrollArea className="h-[400px]">
                  <div className="space-y-3 pr-2">
                    {filteredTransactions.map((transaction) => (
                      <div key={transaction.id} className="flex items-center justify-between rounded-2xl border border-pink-200 bg-white px-4 py-3 shadow-sm">
                        <div className="flex items-center gap-3">
                          <span className="text-2xl">{getCategoryEmoji(transaction.category)}</span>
                          <div>
                            <p className="font-semibold text-pink-700">{transaction.description || 'Untitled'}</p>
                            <p className="text-xs text-pink-500">{transaction.date} • {transaction.category}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <p className={`font-bold ${transaction.amount > 0 ? 'text-green-600' : 'text-pink-600'}`}>
                            {formatCurrency(Math.abs(transaction.amount))}
                          </p>
                          <Button variant="ghost" size="sm" onClick={() => deleteTransaction(transaction.id)} className="text-pink-600 hover:text-pink-700">
                    ...
                    {filteredTransactions.length === 0 && (
                      <p className="text-center text-sm text-pink-500">No matching entries found bestie ✨</p>
                    )}
                  </div>
                </ScrollArea>
              </CardContent>
                    ...
                    <TableBody>
                      {goals.map((goal) => (
                        <TableRow key={goal.id}>
                          ...
                          <TableCell>
                            <div className="w-full bg-pink-200/40 rounded-full h-2.5 dark:bg-pink-900/50 overflow-hidden">
                              <div
          ...
          <TabsContent value="insights">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Spending mood this week</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="mb-4 flex gap-2">
                    <Button size="sm" variant={spendingPeriod === 'weekly' ? 'default' : 'outline'} onClick={() => setSpendingPeriod('weekly')}>
                      Weekly
                    </Button>
                    <Button size="sm" variant={spendingPeriod === 'monthly' ? 'default' : 'outline'} onClick={() => setSpendingPeriod('monthly')}>
                      Monthly
                    </Button>
                  </div>
                  <Bar data={getSpendingMoodData()} options={{ responsive: true }} />
                </CardContent>
              </Card>
              <Card>
                <CardHeader>
                  ...
                  <div className="mt-6 text-lg space-y-4">
                    ...
                    {financialHealthScore >= 40 && financialHealthScore < 60 && (
                      <>
                        ...
                        <ul className="list-disc list-inside pl-4 space-y-2">
                          <li>Create a detailed budget to understand where every rupee is going</li>
          ...
          <div className="border-t pt-4 mt-4">
            ...
            <div className="flex space-x-4">
              ...
              <Label htmlFor="import-file" className="cursor-pointer">
                <div className="flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-pink-500 hover:bg-pink-600">
      ...
      <Dialog open={showAddTransactionModal} onOpenChange={setShowAddTransactionModal}>
        <DialogContent className="bg-white text-black sm:max-w-2xl data-[state=open]:animate-in data-[state=open]:zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:zoom-out-95">
          <DialogHeader>
            <DialogTitle>Add a transaction bestie ✨</DialogTitle>
            <DialogDescription>Log your latest money moment</DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 py-2">
            <Input
              placeholder="Title (e.g., iced coffee ☕)"
            ...
            >
              ...
              <SelectContent>
                {categories.map((category) => (
                  <SelectItem key={category} value={category}>{getCategoryEmoji(category)} {category}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <Select
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowAddTransactionModal(false)}>Cancel</Button>
            <Button onClick={addTransaction} className="bg-pink-500 hover:bg-pink-600 text-white">Add Transaction 💖</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
